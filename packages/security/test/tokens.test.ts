...
describe('Utils: tokens', function () {
    ...
    it('extract', function () {
        ...
        const parts = security.tokens.resetToken.extract({
            token: token
        });

        assert.notEqual(parts, false);
        if (parts !== false) {
            assert.equal(parts.email, email);
            assert.equal(parts.expires, expires);
            assert.equal((parts as Record<string, unknown>).password, undefined);
            assert.equal((parts as Record<string, unknown>).dbHash, undefined);
        }
    });

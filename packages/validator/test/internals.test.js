...
describe("Validator internals", function () {
    describe("validate()", function () {
        ...
        it("returns type-specific message for isLength", function () {
            ...
            assert.equal(
                errors[0].message,
                "Value in [users.name] exceeds maximum length of 2 characters.",
            );
    ...
    describe("is-email", function () {
        it("supports display name options", function () {
            ...
            assert.equal(
                isEmail("Bad;Name <member@example.com>", { allow_display_name: true }),
                false,
            );
            assert.equal(
                isEmail('"Bad"Name" <member@example.com>', { allow_display_name: true }),
        ...
        it("supports domain specific validation and max length options", function () {
            ...
            assert.equal(
                isEmail("abc123@googlemail.com", { domain_specific_validation: true }),
                true,
            );

...
function colorize(colors, value) {
    if (isArray(colors)) {
        ...
        return (
            "\x1B[" +
            _private.colors[colors][0] +
            "m" +
            value +
            "\x1B[" +
            _private.colors[colors][1] +
            "m"
        );
    }
}
    ...
    _transform(data, enc, cb) {
        ...
        if (data.time) {
            ...
            if (
                ...
                /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(data.time)
            ) {
                time = data.time;
            } else {
        ...
        each(
            Object.fromEntries(Object.entries(data).filter(([key]) => !OMITTED_KEYS.includes(key))),
            function (value, key) {

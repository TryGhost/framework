...
describe("Errors", function () {
    ...
    describe("Inherit from other error", function () {
        ...
        it("has nested object", function () {
            ...
            const ghostError = new errors.InternalServerError({ err: someError }) as Error & {
                obj?: { a: string };
            };
        ...
        it("does not overwrite key attributes", function () {
            const someError = new Error() as Error & {
                errorType?: string;
                statusCode?: number;
                level?: string;
                code?: string;
    ...
    describe("Serialization", function () {
        ...
        it("cannot serialize nothing", function () {
            assert.equal(
                (utils.serialize(undefined as any) as any).message,
        ...
        it("deserializing nothing results in a plain InternalServerError (the default)", function () {
            ...
            assert.equal(
                utils.deserialize({ errors: null as any }).message,
                "The server has encountered an error.",
            );
            assert.equal(
                utils.deserialize({ errors: [] as any[] }).message,
                "The server has encountered an error.",
            );
        });
    ...
    describe("ErrorTypes", function () {
        const expectations: Array<
            [GhostErrorConstructor, Partial<GhostError> & { message: string }]
        > = [
            [
            ...
            [
                errors.RequestNotAcceptableError,
                {
                    statusCode: 406,
                    level: "normal",
            ...
            [
                errors.RangeNotSatisfiableError,
                {
                    statusCode: 416,
                    level: "normal",
            ...
            [
                errors.PasswordResetRequiredError,
                {
                    statusCode: 401,
                    level: "normal",
